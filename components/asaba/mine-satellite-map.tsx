...
type MineSatelliteMapProps = {
  variant?: "full" | "dashboard"
}

export function MineSatelliteMap({ variant = "full" }: MineSatelliteMapProps = {}) {
  ...
  return (
    <div className={cn(isDashboard ? "h-full min-h-[430px]" : "px-3 py-3 lg:px-5")}>
      <section
        className={cn(
          "relative overflow-hidden rounded-lg border border-slate-800 bg-slate-950 shadow-xs",
          isDashboard ? "h-full min-h-[430px]" : "min-h-[calc(100vh-6.25rem)]",
        )}
      >
